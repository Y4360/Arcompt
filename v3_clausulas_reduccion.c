		...
		#pragma omp parallel private(sigma,i,j) shared(a,x,b,x_new,n) num_threads(c)
		{
			...
			#pragma omp for
				...
				for(i = 0; i < n; i++){
					...
					// Cada hilo crea una copia privada de norm2 inicializada en 0
					// Cada hilo suma sus resultados de forma local en cada iteración
					// Al final del bloque paralelo, openmp combina las copias privadas en la variable global
