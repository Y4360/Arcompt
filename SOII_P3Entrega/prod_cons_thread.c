 ...
 *      por lo que el buffer y las sumas de elementos leidos son
 *      simplemente variables globales. NO se necesita mmap().
 *   3. El hilo principal espera a que ambos threads terminen
...
int remove_item(int *cond1, int *cond2){
    for(int i = siguiente_posicion; i < N; i++){       /* recorre de la última posición con hueco hasta N*/
        if((buffer[i] != 0) && (buffer[i] != 100)){    /* busca el primer elemento válido         */
           ...
           *cond2 = 0; 					/* Todavía quedan elementos en el buffer  */
    ...
    for(int i = 0; i < siguiente_posicion; i++){       /* recorre los índices que faltan          */
        if((buffer[i] != 0) && (buffer[i] != 100)){    /* busca el primer elemento válido         */
            ...
            *cond2 = 0;                            /* todavía quedan elementos en la pila FIFO*/
...
void buffer_lleno(){
	...
	while(buffer[i] != 0){   /* Solo saldremos del bucle al encontrar un hueco en el buffer */
		i++;
		if(i >= N) i = 0; /* Al recorrer todo el buffer, volvemos al principio           */
	}
	return;
}
...
/* ------------------------------------------------------------
 * buffer_vacio()
 *   Se hace una espera activa hasta que se encuentre algún entero distinto en el buffer.
 * ------------------------------------------------------------ */
void buffer_vacio(){
	int i = 0;
	while(buffer[i] == 0){   /* Solo saldremos del bucle al encontrar un hueco ocupado en el buffer */
		i++;
		if(i >= N) i = 0; /* Al recorrer todo el buffer, volvemos al principio                   */
	}
	return;
...
void *hilo_productor(void *arg){
    ...
    while(iter < MAX_ITER && entero != 101){
        /* === SECCIÓN CRÍTICA ======================================= */
        entero = produce_item(archivo); /* Leer el un entero del archivo y se suma a la variable global */

	buffer_lleno(); /* Esperamos mientras el buffer está lleno, porque no podemos insertar datos */

	/* Retardo de velocidad dentro de la región crítica */
	...
	insert_item(entero); /* inserta el entero en el buffer compartido   */
        ...
        iter++; /* avanza el contador de iteraciones del productor */
    }
...
void *hilo_consumidor(void *arg){
     ...
     * ----------------------------------------------------------- */
    while(!(eof_detectado && fin_pila) && iter < MAX_ITER){

	buffer_vacio(); /* esperamos mientras el buffer está vacío */
	...
	if(item == 101) break;
        ...
        iter++; /* avanza el contador de iteraciones del consumidor */
    }
...
// gcc -o prod_cons_thread prod_cons_thread.c -pthread
// ./prod_cons_thread
