	...
	for(int iter = 0; iter < MAXITER; iter++){
		...
		for(int i = 0; i < n; i+=4){
			...
			for(int j = 0; j < n; j+=4){
				...
				sigma += a[i+1][j+2] * x[j+2];
				sigma += a[i+1][j+3] * x[j+3];
				
				sigma += a[i+2][j] * x[j];
				sigma += a[i+2][j+1] * x[j+1];
				sigma += a[i+2][j+2] * x[j+2];
				sigma += a[i+2][j+3] * x[j+3];

				sigma += a[i+3][j] * x[j];
				sigma += a[i+3][j+1] * x[j+1];
				sigma += a[i+3][j+2] * x[j+2];
				sigma += a[i+3][j+3] * x[j+3];
			}

			// Calculamos el nuevo elemento de x
			x_new[i] = (b[i] - sigma) / a[i][i];
			x_new[i+1] = (b[i+1] - sigma) / a[i+1][i+1];
			x_new[i+2] = (b[i+2] - sigma) / a[i+2][i+2];
			x_new[i+3] = (b[i+3] - sigma) / a[i+3][i+3];
