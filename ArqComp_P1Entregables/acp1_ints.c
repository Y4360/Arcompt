...
int main(int argc, char **argv) {
    ...
    /* Datos requeridos */
    int S[10];              // Array de resultados
    int *A;                 // Puntero a los enteros a estudiar
    ...
    int R, D;               // Datos para estudiar la localidad
    int i, j;               // Índices para los bucles
    int *ind;               // Array para el acceso indirecto a los valores de A
    ...
    if(D < 16){
        R = (16 * R) / D;    // Hacen falta 16/D saltos para cubrir una línea completa
    }
    // En el caso en el que D >= 16, con cada salto ya se accede a una línea caché diferente, con lo que R = L


    /* Inicializamos datos */
    ind = (int *) malloc(R * sizeof(int)); // R accesos indirectos a valores de A
    for(i = 0; i < R; i++){
        ind[i] = i*D;                      // Escogemos datos de D en D
    }

    A = (int*) aligned_alloc(64, R * D * sizeof(int));  // R*D doubles en rangos [1,2) o (-2,-1], alineando comienzo de A con el inicio de línea caché
    for(i = 0; i < R*D; i++){
        A[i] = (((int) rand()) / RAND_MAX) + 1; // Valores aleatorios entre 0 y 1 trasladados a 1 y 2
